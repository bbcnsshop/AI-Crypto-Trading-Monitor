...
class AIEngine(models.Model):
    ...
    def _create_category_mapping(self, product_name, category_id, confidence):
        ...
        existing = self.env['multichannel.ai.category.mapping'].search([('product_name', '=', product_name), ('category_id', '=', category_id)], limit=1)
        if not existing:
            self.env['multichannel.ai.category.mapping'].create({'engine_id': self.id, 'product_name': product_name, 'category_id': category_id, 'confidence': confidence, 'mapping_count': 1})
        else:
...
class AICategoryMapping(models.Model):
    _name = 'multichannel.ai.category.mapping'
    _description = 'AI Category Mapping'
    engine_id = fields.Many2one('multichannel.ai.engine', string='AI Engine')
